...
<title>Stav zásob alkoholu PDF</title>
  <style type="text/css">
...
<tr>
<td>EuroSecom  -  Stav zásob alkoholu PDF 
...
$sqlt = 'DROP TABLE F'.$kli_vxcf.'_sklprcalk'.$kli_uzid;
$vysledok = mysql_query("$sqlt");
...
$vsql = 'CREATE TABLE F'.$kli_vxcf.'_sklprcalk'.$kli_uzid.$sqlt;
$vytvor = mysql_query("$vsql");
...
$dsqlt = "INSERT INTO F$kli_vxcf"."_sklprcalk$kli_uzid".
...
//vypocitaj hodnotu
$sqlt = "UPDATE F".$kli_vxcf."_sklprcalk".$kli_uzid." SET hod=zas*cen";
//echo $sqlt;
$vysledok = mysql_query("$sqlt");
...
{ 
$dsqlt = "INSERT INTO F$kli_vxcf"."_sklprcalk$kli_uzid".
" SELECT druh,1,0,ume,dat,skl,cis,'',mno,cen,SUM(zas),SUM(hod),SUM(vdj),SUM(prj),SUM(pcs) FROM F$kli_vxcf"."_sklprcalk$kli_uzid ".
" GROUP BY skl,cis".
"";
$dsql = mysql_query("$dsqlt");

$sqlt = "DELETE FROM F".$kli_vxcf."_sklprcalk".$kli_uzid." WHERE pox1 = 0";
$vysledok = mysql_query("$sqlt");

$dsqlt = "UPDATE F$kli_vxcf"."_sklprcalk$kli_uzid  SET pox1=0 "; 
$dsql = mysql_query("$dsqlt");

$dsqlt = "UPDATE F$kli_vxcf"."_sklprcalk$kli_uzid  SET cen=hod/zas WHERE zas != 0 "; 
$dsql = mysql_query("$dsqlt");
}
...
$dsqlt = "UPDATE F$kli_vxcf"."_sklprcalk$kli_uzid,F$kli_vxcf"."_sklcisudaje ".
" SET pox1=99, ean=xnat ".
" WHERE F$kli_vxcf"."_sklprcalk$kli_uzid.cis=F$kli_vxcf"."_sklcisudaje.xcis AND xdr1 = 1 "; 
$dsql = mysql_query("$dsqlt");

$sqlt = "DELETE FROM F".$kli_vxcf."_sklprcalk".$kli_uzid." WHERE pox1 != 99 ";
$vysledok = mysql_query("$sqlt");
...
$dsqlt = "INSERT INTO F$kli_vxcf"."_sklprcalk$kli_uzid".
" SELECT 0,0,9,ume,dat,skl,cis,'',mno,cen,SUM(zas),SUM(hod),SUM(vdj),SUM(prj),SUM(pcs) FROM F$kli_vxcf"."_sklprcalk$kli_uzid ".
" GROUP BY skl".
"";
$dsql = mysql_query("$dsqlt");
...
$dsqlt = "INSERT INTO F$kli_vxcf"."_sklprcalk$kli_uzid".
" SELECT 0,0,10,ume,dat,99999999,cis,'',mno,cen,SUM(zas),SUM(hod),SUM(vdj),SUM(prj),SUM(pcs) FROM F$kli_vxcf"."_sklprcalk$kli_uzid ".
...
if ( $copern == 20 OR $copern == 10 OR $copern == 30 )
  {
$sqltt = "SELECT * FROM F$kli_vxcf"."_sklprcalk$kli_uzid".
" LEFT JOIN F$kli_vxcf"."_sklcis".
" ON F$kli_vxcf"."_sklprcalk$kli_uzid.cis=F$kli_vxcf"."_sklcis.cis".
" LEFT JOIN F$kli_vxcf"."_skl".
" ON F$kli_vxcf"."_sklprcalk$kli_uzid.skl=F$kli_vxcf"."_skl.skl".
" WHERE hod != 0".
" ORDER BY F$kli_vxcf"."_sklprcalk$kli_uzid.skl,pox,druh,F$kli_vxcf"."_sklprcalk$kli_uzid.cis,F$kli_vxcf"."_sklprcalk$kli_uzid.cen";
  }
...
if ( $copern == 10 )  { $pdf->Cell(90,6,"Stav zásob alkoholu Sklad $riadok->skl $riadok->nas","LTB",0,"L"); }
...
$pdf->Cell(10,5,"SKL","1",0,"R");$pdf->Cell(20,5,"MAT","1",0,"R");$pdf->Cell(90,5,"EAN Názov","1",0,"L");
$pdf->Cell(20,5,"Cena za MJ","1",0,"R");$pdf->Cell(10,5,"MJ","1",0,"L");
$pdf->Cell(15,5,"Zásoba MJ","1",0,"R");$pdf->Cell(0,5,"Hodnota","1",1,"R");
...
$pdf->Cell(10,5,"$riadok->skl","0",0,"R");$pdf->Cell(20,5,"$riadok->cis","0",0,"R");$pdf->Cell(90,5,"$riadok->ean $riadok->nat","0",0,"L");
$pdf->Cell(20,5,"$riadok->cen","0",0,"R");$pdf->Cell(10,5,"$riadok->mer","0",0,"L");
$pdf->Cell(15,5,"$riadok->zas","0",0,"R");$pdf->Cell(0,5,"$riadok->hod","0",1,"R");
