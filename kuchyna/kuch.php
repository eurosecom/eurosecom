...
//echo "hotel".$hotel;
$xkuch=1;
$sqlttt = "SELECT * FROM ".$mysqldbdata.".F$vyb_xcf"."_hoteluzid WHERE uzix=$kli_uzid ";
$sqlmax = mysql_query("$sqlttt");
  if (@$zaznam=mysql_data_seek($sqlmax,0))
  {
  $riadmax=mysql_fetch_object($sqlmax);
  $xkuch=1*$riadmax->xkuch;
  }
if( $xkuch == 0 )
{
?>
...
<?php if( $newmenu == 0 ) { ?>
  ...
  // PRVE MENU
  var M1 = new CSkryvaneMenu("M1", 420);
  M1.NovaPolozka
  ( "Jedálny lístok, stravnı a normovacích list, spotreba",
    "ZozObj()",
    "Zoznam jedálnych lístkov, stravnıch listov,  normovanıch a skutoènıch spotrieb objednanıch jedál z denného menu alebo jedálneho lístka..."
  );
  M1.NovaPolozka
  ( "Dodacie listy jedál a vırobkov odberate¾om",
    "ZozDod()",
    "Zoznam Dodacích listov na dodané tovarové poloky a jedlá odberate¾om..."
  );
  M1.NovaPolozka
  ( "Zoznam rozpracovanıch objednávok jedál v kuchyni",
    "ZozRoz()",
    "Objednávky z reštaurácie do kuchyne a ich aktuálny stav..."
  );
  M1.NovaPolozka
  ( "Jedálne lístky na WEB stránke",
    "Jedweb()",
    "Umiestnenie PDF jedálneho lístka na web stránku..."
  );
  M1.classMenu = "menu2";
  ...
  M2.NovaPolozka
  ( "Preh¾ad o èerpaní finanèného limitu",
    "FinLimit()",
    "Preh¾ad o èerpaní finanèného limitu"
  );
  M2.NovaPolozka
  ( "Vırobné náklady a trby v kuchyni pod¾a dní",
    "VyrSpotKUCH()",
    "Vytvorenie zostavy o vırobnıch nákladoch a trbách z predaja v KUCHYNI za jednotlivé dni"
  );
  M2.NovaPolozka
  ( "Vırobné náklady a trby v kuchyni pod¾a akcií",
    "VyrSpotKUCH2()",
    "Vytvorenie zostavy o vırobnıch nákladoch a trbách z predaja v KUCHYNI za jednotlivé akcie v dòoch"
  );
  M2.NovaPolozka
  ( "Vırobné náklady na suroviny pod¾a jedál",
    "VyrSpotKUCH3()",
    "Vytvorenie zostavy o vırobnıch nákladoch na suroviny za jednotlivé jedlá"
  );
  M2.NovaPolozka
  ( "Vırobné náklady na suroviny pod¾a dní",
    "VyrSpotDni()",
    "Vytvorenie zostavy o vırobnıch nákladoch na suroviny za jednotlivé dni v mesiaci"
  );
  M2.NovaPolozka
  ( "Cena porcie a cena surovín pod¾a jedál",
    "VyrSpotKUCH4()",
    "Vytvorenie zostavy o priemernej cene a celkovej cene surovín za jednotlivé jedlá"
  );
  M2.classMenu = "menu2";
  ...
  ( "Receptúry jedál",
    "Receptury()",
    "Zostavenie receptúr jedál , zadanie mnostva surovín pouívanıch pri vırobe"
  );
  M4.NovaPolozka
  ( "Vırobné suroviny",
    "Komponenty()",
    "Zoznam surovín pouívanıch v receptúrach"
  );
  M4.NovaPolozka
  ( "Èíselník predávaného tovaru",
    "OtvorCcis()",
    "Èíselník predávanıch jedál, nápojov a tovaru"
  );
  M4.NovaPolozka
...
<?php                     } ?>
  ...
  function OtvorUhot41()
  { 
  var okno = window.open("../kuchyna/uhot.php?copern=41","_blank");
  }
...
//[[[[[[[ body vyroba
  ...
  function SpoKomp()
  { 
  var okno = window.open("../kuchyna/vyrspot.php?copern=30&drupoh=1&page=1&spo=1","_blank",'<?php echo $tlcswin; ?>');
  }

  function VyrSpotKUCH()
  { 
  var okno = window.open("../kuchyna/spot_trzba.php?copern=10&drupoh=1&page=1&spo=0&pds=1","_blank",'<?php echo $tlcswin; ?>');
  }
  function VyrSpotKUCH2()
  { 
  var okno = window.open("../kuchyna/spot_trzbaakcie.php?copern=10&drupoh=1&page=1&spo=0&pds=2","_blank",'<?php echo $tlcswin; ?>');
  }
  function VyrSpotKUCH3()
  { 
  var okno = window.open("../kuchyna/spot_trzbajedla.php?copern=20&drupoh=1&cislo_dok=1&spotreba=1&druh=1","_blank",'<?php echo $tlcswin; ?>');
  }
  function VyrSpotDni()
  { 
  var okno = window.open("../kuchyna/spot_trzbadni.php?copern=20&drupoh=1&cislo_dok=1&spotreba=1&druh=1","_blank",'<?php echo $tlcswin; ?>');
  }
  function VyrSpotKUCH4()
  { 
  var okno = window.open("../kuchyna/spot_trzbajedla.php?copern=20&drupoh=1&cislo_dok=1&spotreba=1&druh=1&ceny=1","_blank",'<?php echo $tlcswin; ?>');
  }
  function FinLimit()
  { 
  var okno = window.open("../kuchyna/spot_trzbaakcie.php?copern=10&drupoh=1&page=1&spo=0&pds=2&finlim=1","_blank",'<?php echo $tlcswin; ?>');
  }

  function ZozObj()
  { 
  var okno = window.open("../kuchyna/vstobj.php?copern=1&drupoh=1&page=1&spo=0&pds=0","_blank",'<?php echo $parwin; ?>');
  }

  function ZozDod()
  { 
  var okno = window.open("../kuchyna/vstobj.php?copern=1&drupoh=2&page=1&spo=0&pds=0","_blank",'<?php echo $parwin; ?>');
  }

  function Jedweb()
  { 
  var okno = window.open("../jedalnylistok/jedalnylistok.php?copern=1&drupoh=2&page=1&prihl=1&cfir=<?php echo $vyb_xcf; ?>","_blank",'<?php echo $parwin; ?>');
  }
  ...
  function Receptury()
  { 
  var okno = window.open("../kuchyna/cisreceptov.php?copern=1&drupoh=1&page=1","_blank",'<?php echo $parwin; ?>');
  }

  function Komponenty()
  { 
  var okno = window.open("../kuchyna/cissurovin.php?copern=1&drupoh=1&page=1","_blank",'<?php echo $parwin; ?>');
  }
...
htmlmenu += "<a href=\"#\" onClick=\"window.open('../kuchyna/vstobj.php?copern=1&drupoh=1&page=1&spo=0&pds=0', '_blank','<?php echo $parwin; ?>' )\">" +
"Chcete zoznam normovanıch objednávok ?</a>";
    htmlmenu += "</td></tr>";

    htmlmenu += "<tr><td width='100%' class='ponuka' colspan='2'>"; 
htmlmenu += "<a href=\"#\" onClick=\"window.open('../kuchyna/vstobj.php?copern=1&drupoh=2&page=1&spo=0&pds=0', '_blank','<?php echo $parwin; ?>' )\">" +
"Chcete zoznam dodacích listov ?</a>";
    htmlmenu += "</td></tr>";

    htmlmenu += "<tr><td width='100%' class='ponuka' colspan='2'>"; 
htmlmenu += "<a href=\"#\" onClick=\"window.open('../faktury/vstfak.php?copern=1&drupoh=1&page=1&pocstav=0', '_blank','<?php echo $parwin; ?>' )\">" +
"Chcete zoznam odberate¾skıch faktúr ?</a>";
    htmlmenu += "</td></tr>";

    htmlmenu += "<tr><td width='100%' class='ponuka' colspan='2'>"; 
htmlmenu += "<a href=\"#\" onClick=\"window.open('../ucto/vstpok.php?copern=1&drupoh=1&page=1', '_blank','<?php echo $parwin; ?>' )\">" +
"Chcete zoznam príjmovıch pokladniènıch dokladov ?</a>";
    htmlmenu += "</td></tr>";

    htmlmenux = "<tr><td width='100%' class='ponuka' colspan='2'>"; 
htmlmenux += "<a href=\"#\" onClick=\"window.open('../kuchyna/nefakdod.php?copern=10&drupoh=1&page=1', '_blank','<?php echo $parwin; ?>' )\">" +
"Chcete vyfaktúrova dodacie listy a dopravu ?</a>";
    htmlmenux += "</td></tr>";
...
<tr>
<td width="60%" >
<a href='kuch.php?copern=22&newmenu=<?php echo "$newmenu";?>'><img src='../obr/uprav.png' width=15 height=10 border=0 alt="Zmena úètovnej jednotky"> Firma</a>
<?php echo "$vyb_xcf";?> <?php echo "$vyb_naz";?>
</td>
<td width="40%" >
<a href='kuch.php?copern=24&newmenu=<?php echo "$newmenu";?>'><img src='../obr/uprav.png' width=15 height=10 border=0 alt="Zmena úètovného obdobia"> Úètovné obdobie:</a>
<?php echo "$vyb_ume";?>
</td>
</tr>
</table>
...
<a href="#" >Jedálny lístok, stravnı a normovacích list, spotreba</a></td></tr>
<tr><td width='100%'  colspan='2' onClick="ZozDod();" onmouseover="MZvyrazni(this);" onmouseout="MNeZvyrazni(this);">
<a href="#" >Dodacie listy jedál a vırobkov odberate¾om</a></td></tr>
<tr><td width='100%'  colspan='2' onClick="ZozRoz();" onmouseover="MZvyrazni(this);" onmouseout="MNeZvyrazni(this);">
<a href="#" >Zoznam rozpracovanıch objednávok jedál v kuchyni</a></td></tr>
<tr><td width='100%'  colspan='2' onClick="Jedweb();" onmouseover="MZvyrazni(this);" onmouseout="MNeZvyrazni(this);">
<a href="#" >Jedálny lístok n WEbe</a></td></tr>
</FORM>
</table>
</div>
...
<table class='pmenu' width='100%'>
<FORM name='znew' method='post' action='#' >
<tr><td width='90%' onClick='M2ZhasniMenu();'>Menu Informácie a vıstupy</td>
<td width='10%' align='right'>
<img border=0 src='../obr/zmazuplne.png' style='width:10; height:10;' onClick='M2ZhasniMenu();' alt='Zhasni menu' ></td></tr>  

<tr><td width='100%' colspan='2' onClick="FinLimit();" onmouseover="MZvyrazni(this);" onmouseout="MNeZvyrazni(this);">
<a href="#" >Preh¾ad o èerpaní finanèného limitu</a></td></tr>
<tr><td width='100%'  colspan='2' onClick="VyrSpotKUCH();" onmouseover="MZvyrazni(this);" onmouseout="MNeZvyrazni(this);">
<a href="#" >Vırobné náklady a trby v kuchyni pod¾a dní</a></td></tr>
<tr><td width='100%'  colspan='2' onClick="VyrSpotKUCH2();" onmouseover="MZvyrazni(this);" onmouseout="MNeZvyrazni(this);">
<a href="#" >Vırobné náklady a trby v kuchyni pod¾a akcií</a></td></tr>
<tr><td width='100%' colspan='2' onClick="VyrSpotKUCH3();" onmouseover="MZvyrazni(this);" onmouseout="MNeZvyrazni(this);">
<a href="#" >Vırobné náklady na suroviny pod¾a jedál</a></td></tr>
<tr><td width='100%'  colspan='2' onClick="VyrSpotKUCH4();" onmouseover="MZvyrazni(this);" onmouseout="MNeZvyrazni(this);">
<a href="#" >Cena porcie a cena surovín pod¾a jedál</a></td></tr>
</FORM>
</table>
</div>
...
<FORM name="fir1" class="obyc" method="post" action="kuch.php?copern=25&newmenu=<?php echo "$newmenu";?>" >
<tr>
<td class="obyc">
...
<span style="position:absolute; top:70; left:10; ">
<table class="fmenu" width="30%" >
<FORM name="fir1" class="obyc" method="post" action="kuch.php?copern=23&newmenu=<?php echo "$newmenu";?>" >
<tr>
<td class="obyc">
...
$robot=1;
$absolut=1;
$cislista = include("kuch_lista.php");
