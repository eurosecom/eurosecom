...
if( confirm ("Chcete naËÌtaù ˙hrady dod·vateæsk˝ch fakt˙r  ?") )
         { window.open('vykaz_fin112_citajdod.php?cislo_oc=<?php echo $cislo_oc;?>&copern=20&drupoh=1&page=1&subor=0&strana=2&xx=1', '_self' );  }
...
echo "naËÌtanie Dod·vateæsk˝ch ume ".$kli_vume."<br />";
...
$dsqlt = "DELETE FROM F$kli_vxcf"."_prcuhr$kli_uzid WHERE LEFT(ucm,3) != 321 ";
$dsql = mysql_query("$dsqlt");

$dsqlt = "DELETE FROM F$kli_vxcf"."_prcuhr$kli_uzid WHERE LEFT(ucd,3) != 211 AND LEFT(ucd,3) != 221 ";
$dsql = mysql_query("$dsqlt");


$sqltt = "SELECT * FROM F".$kli_vxcf."_prcuhr$kli_uzid WHERE dok >= 5001 AND dok <= 5020 ORDER BY dok, cpl ";
...
$jeuctdod=0;
//uctdod
$sqlt2 = "SELECT * FROM F".$kli_vxcf."_uctdod WHERE ico = $rtov->ico AND fak = $rtov->fak AND rdp < 300 AND ( ucd = 111100 OR ucd = 131100 OR ucd = 395000 OR LEFT(ucd,1) = 6 ) ORDER BY dok, cpl ";
...
$sqlt3 = "SELECT * FROM ".$databaza18."F".$kli_vxcfmin."_uctdod WHERE ico = $rtov->ico AND fak = $rtov->fak AND rdp < 300 AND ( ucd = 111100 OR ucd = 131100 OR ucd = 395000 OR LEFT(ucd,1) = 6 ) ORDER BY dok, cpl ";
...
if( $jeuctdod == 0 ) { echo $rtov->dok.";".$rtov->ico.";".$rtov->fak.";".";_____NENASIEL"."<br />"; }
...
//$hodsum=463.76+$hodmd-$hoddl;
//echo "zostatok ".$hodsum.";sum"."<br />";

$dsqlt = "DELETE FROM F$kli_vxcf"."_prcrzp$kli_uzid WHERE LEFT(ucm,3) = 395 AND LEFT(ucd,3) = 395 ";
$dsql = mysql_query("$dsqlt");
$dsqlt = "DELETE FROM F$kli_vxcf"."_prcrzp$kli_uzid WHERE LEFT(ucm,3) = 343 AND LEFT(ucd,3) = 395 ";
//$dsql = mysql_query("$dsqlt");
$dsqlt = "DELETE FROM F$kli_vxcf"."_prcrzp$kli_uzid WHERE LEFT(ucm,3) = 395 AND LEFT(ucd,3) = 343 ";
//$dsql = mysql_query("$dsqlt");

$dsqlt = "UPDATE F$kli_vxcf"."_prcrzp$kli_uzid SET hor=-1*hor WHERE LEFT(ucd,1) = 6 ";
$dsql = mysql_query("$dsqlt");
...
$sqltt = "SELECT * FROM F".$kli_vxcf."_prcrzps$kli_uzid WHERE dok >= 0 AND hor-hou != 0";

$sqltt = "SELECT * FROM F".$kli_vxcf."_prcrzps$kli_uzid WHERE dok >= 0 ORDER BY dou, cpl";

$sqltt = "SELECT * FROM F".$kli_vxcf."_prcrzps$kli_uzid WHERE dok >= 0 ORDER BY rzp";

//$sqltt = "SELECT * FROM F".$kli_vxcf."_prcrzp$kli_uzid WHERE dok >= 0 ORDER BY dou, cpl";
...
$sqltt3 = "UPDATE F".$kli_vxcf."_uctvykaz_fin104 SET skutocnost=0 WHERE cpl >= 0  ";
