...
//andrej
//starsie ako 1.1. zrataj do jedneho riadku PS
$dat0101=$kli_vrok."-01-01";

$dsqlt = "UPDATE F$kli_vxcf"."_prsaldoicofaknesp$kli_uzid SET fak=1111, ico=44551142 WHERE dat < '$dat0101' ";
$dsql = mysql_query("$dsqlt");

$dsqlt = "INSERT INTO F$kli_vxcf"."_prsaldoicofaknesp$kli_uzid".
" SELECT 0,0,998,drupoh,uce,0,ume,dat,das,daz,dok,ico,fak,".
"poz,ksy,ssy,SUM(hdp),SUM(hdu),SUM(hod),SUM(uhr),SUM(zos),dau,0".
" FROM F$kli_vxcf"."_prsaldoicofaknesp$kli_uzid".
" WHERE pox = 1 AND pox3 = 0 AND fak = 1111 AND ico = 44551142 AND dat < '$dat0101' ".
" GROUP BY pox ";
$dsql = mysql_query("$dsqlt");

$dsqlt = "DELETE FROM F$kli_vxcf"."_prsaldoicofaknesp$kli_uzid WHERE pox = 1 AND pox3 = 0 AND fak = 1111 AND ico = 44551142 AND dat < '$dat0101'";
$dsql = mysql_query("$dsqlt");

$dsqlt = "UPDATE F$kli_vxcf"."_prsaldoicofaknesp$kli_uzid SET zos=hod-uhr WHERE pox = 998 ";
$dsql = mysql_query("$dsqlt");

$dsqlt = "UPDATE F$kli_vxcf"."_prsaldoicofaknesp$kli_uzid SET pox = 1 WHERE pox = 998 ";
$dsql = mysql_query("$dsqlt");

//koniec starsie ako 1.1. zrataj do jedneho riadku PS
...
" WHERE uuce = $h_uce AND uduh >= '$dat0101' ";
//echo $dsqlt;
$dsql = mysql_query("$dsqlt");
//exit;
...
if( $h_obd > 0 AND $h_obd < 13 ) $h_obdn=" ".$datkdph_sk;
...
if( $rtov->pox == 1 )
             {
if( $rtov->pox3 == 0 AND ( $rtov->fak != 1111 OR $rtov->ico != 44551142 ) )
{
$pdf->Cell(6,4," ","0",0,"L");
$pdf->Cell(20,5,"$rtov->fak","0",0,"R");
$pdf->Cell(17,5,"$dat_sk","0",0,"L");$pdf->Cell(10,4," ","0",0,"L");
$pdf->Cell(47,5,"Rechnung","0",0,"L");
$pdf->Cell(20,5," ","0",0,"R");$pdf->Cell(20,4,"$rtov->hod","0",0,"R");$pdf->Cell(0,4," ","0",1,"R");
}

if( $rtov->pox3 == 0 AND $rtov->fak == 1111 AND $rtov->ico == 44551142 )
{

$dat1_sk=SkDatum($dat0101);

$pdf->Cell(6,4," ","0",0,"L");
$pdf->Cell(20,5," ","0",0,"R");
$pdf->Cell(17,5,"$dat1_sk","0",0,"L");$pdf->Cell(10,4," ","0",0,"L");
$pdf->Cell(47,5,"Saldovortrag","0",0,"L");
$pdf->Cell(20,5," ","0",0,"R");$pdf->Cell(20,4,"$rtov->hod","0",0,"R");$pdf->Cell(0,4," ","0",1,"R");
}
...
$pdf->Cell(20,5," ","0",0,"R");
$pdf->Cell(17,5,"$dat_sk","0",0,"L");$pdf->Cell(10,4," ","0",0,"L");
$pdf->Cell(47,5,"Zahlung","0",0,"L");
$pdf->Cell(20,5,"$rtov->hod","0",0,"R");$pdf->Cell(20,4," ","0",0,"R");$pdf->Cell(0,4," ","0",1,"R");
...
if( $sumico == 0 AND $uhr == 0 )
{
$pdf->Cell(25,4,"Saldo","T",0,"L");$pdf->Cell(75,4," ","T",0,"L");
$pdf->Cell(20,4,"$rtov->uhr","T",0,"R");$pdf->Cell(20,4,"$rtov->hod","T",0,"R");$pdf->Cell(20,4,"$rtov->zos","T",1,"R");
$pdf->Cell(90,1,"","0",1,"L");
}
