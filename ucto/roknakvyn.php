...
" WHERE ".$databaza."F$kli_vxcfx"."_$uctovanie.dok=".$databaza."F$kli_vxcfx"."_$doklad.dok AND ( LEFT(ucm,1) = 5 OR  LEFT(ucm,1) = 6 OR LEFT(ucd,1) = 5 OR LEFT(ucd,1) = 6 ) ";
//echo $dsqlt."<br />";
...
" WHERE ( LEFT(ucm,1) = 5 OR LEFT(ucm,1) = 6 OR LEFT(ucd,1) = 5 OR LEFT(ucd,1) = 6 ) ";
$dsql = mysql_query("$dsqlt");
...
" WHERE konx = 0 AND  ( LEFT(ucd,1) = 5 OR LEFT(ucd,1) = 6 )  ";
" ";
$dsql = mysql_query("$dsqlt");

$sqtoz = "DELETE FROM F$kli_vxcf"."_prctopmanx$kli_uzid WHERE LEFT(uce,1) != 5 AND LEFT(uce,1) != 6 ";
...
$sqtoz = "UPDATE F$kli_vxcf"."_prctopman$kli_uzid SET prx=11 WHERE LEFT(uce,1) = 6 ";
$oznac = mysql_query("$sqtoz");

//sumar za naklady
$dsqlt = "INSERT INTO F$kli_vxcf"."_prctopman$kli_uzid "." SELECT".
" 0,9,ume,dat,uce,ucm,ucd,SUM(hod),0, ".
" rrd,'',0,0,sum(jan),sum(feb),sum(mar),sum(apr),sum(maj),sum(jun),sum(jul),sum(aug),sum(sep),sum(okt),sum(nov),sum(der),0,0 ".
" FROM F$kli_vxcf"."_prctopman$kli_uzid".
" WHERE prx = 1  ".
" GROUP BY prx ".
...
//sumar za vynosy
$dsqlt = "INSERT INTO F$kli_vxcf"."_prctopman$kli_uzid "." SELECT".
" 0,19,ume,dat,uce,ucm,ucd,SUM(hod),0, ".
" rrd,'',0,0,sum(jan),sum(feb),sum(mar),sum(apr),sum(maj),sum(jun),sum(jul),sum(aug),sum(sep),sum(okt),sum(nov),sum(der),0,0 ".
" FROM F$kli_vxcf"."_prctopman$kli_uzid".
" WHERE prx = 11  ".
" GROUP BY prx ".
...
//sumar za naklady(-) pre zisk
$dsqlt = "INSERT INTO F$kli_vxcf"."_prctopman$kli_uzid "." SELECT".
" 0,109,ume,dat,uce,ucm,ucd,-SUM(hod),0, ".
" rrd,'',0,0,-sum(jan),-sum(feb),-sum(mar),-sum(apr),-sum(maj),-sum(jun),-sum(jul),-sum(aug),-sum(sep),-sum(okt),-sum(nov),-sum(der),0,0 ".
" FROM F$kli_vxcf"."_prctopman$kli_uzid".
" WHERE prx = 1  ".
" GROUP BY prx ".
...
//sumar zisk
$dsqlt = "INSERT INTO F$kli_vxcf"."_prctopman$kli_uzid "." SELECT".
" 0,20,ume,dat,uce,ucm,ucd,SUM(hod),0, ".
" rrd,'',0,0,sum(jan),sum(feb),sum(mar),sum(apr),sum(maj),sum(jun),sum(jul),sum(aug),sum(sep),sum(okt),sum(nov),sum(der),0,0 ".
" FROM F$kli_vxcf"."_prctopman$kli_uzid".
" WHERE ( prx = 109 OR prx = 11 )  ".
" GROUP BY ccx ".
"";
//echo $dsqlt;
$dsql = mysql_query("$dsqlt");

$sqtoz = "DELETE FROM F$kli_vxcf"."_prctopman$kli_uzid WHERE prx = 109";
$oznac = mysql_query("$sqtoz");

$sqtoz = "UPDATE F$kli_vxcf"."_prctopman$kli_uzid SET rok=jan+feb+mar+apr+maj+jun+jul+aug+sep+okt+nov+der ";
$oznac = mysql_query("$sqtoz");
...
<title>Medziroèné náklady</title>
  <style type="text/css">
...
<td>EuroSecom  -  Medziroèné náklady
...
$pdf->Cell(90,6,"Medziroèný preh¾ad nákladov a výnosov za mesiace $h_obdp. až $h_obdk. ","LTB",0,"L"); 
$pdf->Cell(0,6,"$kli_nxcf strana $strana","RTB",1,"R");

$pdf->SetFont('arial','',10);


$pdf->Cell(35,6,"Úèet","1",0,"R");
...
$pdf->Cell(35,6,"$hlavicka->rrd","0",0,"R");
...
{
$pdf->SetFont('arial','',9);
$pdf->Cell(35,6,"Náklady","BT",0,"L");
$pdf->Cell(35,6,"$xjan","BT",0,"R");$pdf->Cell(35,6,"$xfeb","BT",0,"R");$pdf->Cell(35,6,"$xmar","BT",0,"R");
$pdf->Cell(35,6,"$xapr","BT",0,"R");$pdf->Cell(35,6,"$xmaj","BT",0,"R");
$pdf->Cell(0,6,"$xrok","BT",1,"R");
}
if( $hlavicka->prx == 19 )
{
$pdf->SetFont('arial','',9);
$pdf->Cell(35,6,"Výnosy","BT",0,"L");
$pdf->Cell(35,6,"$xjan","BT",0,"R");$pdf->Cell(35,6,"$xfeb","BT",0,"R");$pdf->Cell(35,6,"$xmar","BT",0,"R");
$pdf->Cell(35,6,"$xapr","BT",0,"R");$pdf->Cell(35,6,"$xmaj","BT",0,"R");
$pdf->Cell(0,6,"$xrok","BT",1,"R");
}
...
{
$pdf->SetFont('arial','',9);
$pdf->Cell(35,6,"ZISK","1",0,"L");
$pdf->Cell(35,6,"$xjan","1",0,"R");$pdf->Cell(35,6,"$xfeb","1",0,"R");$pdf->Cell(35,6,"$xmar","1",0,"R");
$pdf->Cell(35,6,"$xapr","1",0,"R");$pdf->Cell(35,6,"$xmaj","1",0,"R");
$pdf->Cell(0,6,"$xrok","1",1,"R");
}
