...
<tr>
<td class="bmenu" colspan="1">Mesaèný Vymeriavací základ odvodov do SP od 1.7.<?php echo $kli_brok; ?></td><td class="bmenu" colspan="1" align="right"><?php echo $riadok->zaklaf; ?></td>
</tr>
<tr>
...
<tr>
<td class="bmenu" colspan="1">Mesaèný Vymeriavací základ odvodov do ZP od 1.7.<?php echo $kli_brok; ?></td><td class="bmenu" colspan="1" align="right"><?php echo $riadok->zaklaz; ?></td>
</tr>
<tr>
...
$sqldok = mysql_query("SELECT * FROM F$kli_vxcf"."_platbyju".$kli_uzid." ");
  if (@$zaznam=mysql_data_seek($sqldok,0))
	...
	$price=0; $pid="";
        ...
        if( $i ==  6 ) { $name="Mesaèný Vymeriavací základ odvodov do SP od 1.7.$kli_brok"; $price=$r12; $pid = "06.";}
        if( $i ==  7 ) { $name="Mesaèné poistné 35,15% do SP od 1.7.$kli_brok"; $price=$r14; $pid = "07.";}
        if( $i ==  8 ) { $name="Mesaèný Vymeriavací základ odvodov do ZP od 1.7.$kli_brok"; $price=$r16; $pid = "08.";}
        if( $i ==  9 ) { $name="Mesaèné poistné 14% do ZP od 1.7.$kli_brok"; $price=$r18; $pid = "09.";}
        if( $i ==  10 ) { $name="CELKOM MESAÈNÉ POISTNÉ SP a ZP od 1.7.$kli_brok"; $price=$r20; $pid = "10.";}
