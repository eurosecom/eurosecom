...
//datova tabulka
$sql = "SELECT prx6 FROM F$kli_vxcf"."_uctfakuhrdph ";
...
$zmazttt = "DROP TABLE F$kli_vxcf"."_uctfakuhrdphprac$kli_uzid "; 
$zmazane = mysql_query("$zmazttt");

$zmazttt = "CREATE TABLE F$kli_vxcf"."_uctfakuhrdphprac$kli_uzid SELECT * FROM F$kli_vxcf"."_uctfakuhrdph WHERE dok < 0 "; 
$zmazane = mysql_query("$zmazttt");

$sql = "ALTER TABLE F$kli_vxcf"."_uctfakuhrdphprac$kli_uzid ADD cis DECIMAL(15,0) DEFAULT 0 AFTER prx2 ";
$vysledek = mysql_query("$sql");
...
$ulozttt = " INSERT INTO F$kli_vxcf"."_uctfakuhrdphprac$kli_uzid SELECT 0,dok,'','',mno,mer,(cep*mno),0,0,0,cis  ".
" FROM F$kli_vxcf"."_sklfak WHERE dok = $hlavicka->dok AND cis > 0 "; 
if( $jetam361 == 1 ) { $ulozene = mysql_query("$ulozttt"); }
//echo $ulozttt."<br />";

$zmazttt = "DELETE FROM F$kli_vxcf"."_uctfakuhrdph WHERE dok = $hlavicka->dok AND kodt != '' "; 
$zmazane = mysql_query("$zmazttt");
...
//cpld	dok	kodt	dtov	mnot	merj	zkld	sumd	prx1	prx2	cis

$ulozttt = "UPDATE F$kli_vxcf"."_uctfakuhrdphprac$kli_uzid,F$kli_vxcf"."_sklcisudaje SET kodt=xtxt5 ".
" WHERE F$kli_vxcf"."_uctfakuhrdphprac$kli_uzid.cis=F$kli_vxcf"."_sklcisudaje.xcis "; 
$ulozene = mysql_query("$ulozttt");

$ulozttt = " INSERT INTO F$kli_vxcf"."_uctfakuhrdphprac$kli_uzid SELECT 0,dok,kodt,'',SUM(mnot),merj,SUM(zkld),0,0,9,cis  ".
" FROM F$kli_vxcf"."_uctfakuhrdphprac$kli_uzid WHERE dok > 0 GROUP BY dok,kodt,dtov "; 
$ulozene = mysql_query("$ulozttt");

$zmazttt = "DELETE FROM F$kli_vxcf"."_uctfakuhrdphprac$kli_uzid WHERE prx2 != 9 "; 
$zmazane = mysql_query("$zmazttt");

$ulozttt = " INSERT INTO F$kli_vxcf"."_uctfakuhrdph SELECT 0,dok,kodt,'',mnot,merj,zkld,0,0,0  ".
" FROM F$kli_vxcf"."_uctfakuhrdphprac$kli_uzid WHERE dok > 0 "; 
$ulozene = mysql_query("$ulozttt");
...
$ulozttt = " INSERT INTO F$kli_vxcf"."_uctfakuhrdph ( dok,kodt,dtov,mnot,merj,prx1,zkld,sumd ) ".
...
$sqltt = "SELECT SUM(zkld) AS sumz FROM F$kli_vxcf"."_uctfakuhrdph WHERE dok = $cislo_dok ";
$sql = mysql_query("$sqltt"); 
  if (@$zaznam=mysql_data_seek($sql,0))
  {
  $riadok=mysql_fetch_object($sql);
...
<tr>
<td>EuroSecom  -  ⁄hrady fakt˙ry ËÌslo <?php echo $cislo_dok; ?> pre uplatnenie a odpoËet DPH aû po zaplatenÌ
</td>
...
$sqltt = "SELECT * FROM F$kli_vxcf"."_uctfakuhrdph WHERE dok = $cislo_dok ORDER BY cpld";
$sql = mysql_query("$sqltt");
...
<td class="hmenu" width="10%" >KÛd
 <img border=1 src="../obr/info.png" style="width:15; height:15;" title=" " >
