...
(
   ...
   foz          DECIMAL(10,2) DEFAULT 0,
   dnp          DECIMAL(10,2) DEFAULT 0,
   cst          DECIMAL(10,2) DEFAULT 0,
...
//zober hod odpracovane
$dsqlt = "INSERT INTO F$kli_vxcf"."_mzdprcvypl".$kli_uzid.
" SELECT oc,".
" hod,0,0,0,0,0,0,ume,0,0,0,".
" 1".
...
$dsqlt = "INSERT INTO F$kli_vxcf"."_mzdprcvypl".$kli_uzid.
" SELECT oc,".
" 0,0,sum_hru,0,(ofir_np+ofir_sp+ofir_ip+ofir_pn+ofir_up+ofir_rf+ofir_gf+ofir_zp),0,0,ume,(ozam_np+ozam_sp+ozam_ip+ozam_pn+ozam_zp),odan_dnp,0,".
" 1".
...
$sqtoz = "UPDATE F$kli_vxcf"."_mzdprcvypl$kli_uzid SET spo=hru+fon ";
$oznac = mysql_query("$sqtoz");
$sqtoz = "UPDATE F$kli_vxcf"."_mzdprcvypl$kli_uzid SET cst=hru-foz-dnp ";
$oznac = mysql_query("$sqtoz");
...
$dsqlt = "INSERT INTO F$kli_vxcf"."_mzdprcvypl".$kli_uzid.
" SELECT oc,".
" SUM(dni),SUM(dnn),SUM(hru),SUM(nem),SUM(fon),SUM(spo),0,umx,SUM(foz),SUM(dnp),SUM(cst),".
"0".
...
$dsqlt = "INSERT INTO F$kli_vxcf"."_mzdprcvypl".$kli_uzid.
" SELECT oc,".
" SUM(dni),SUM(dnn),SUM(hru),SUM(nem),SUM(fon),SUM(spo),stx,0,SUM(foz),SUM(dnp),SUM(cst),".
"0".
" FROM F$kli_vxcf"."_mzdprcvypl".$kli_uzid.
" WHERE konx2 = 1 ".
" GROUP BY oc,stx".
...
$dsqlt = "INSERT INTO F$kli_vxcf"."_mzdprcvypl".$kli_uzid.
" SELECT oc,".
" SUM(dni),SUM(dnn),SUM(hru),SUM(nem),SUM(fon),SUM(spo),9999999999,0,SUM(foz),SUM(dnp),SUM(cst),".
"1999".
" FROM F$kli_vxcf"."_mzdprcvypl".$kli_uzid.
" WHERE oc > 0 AND konx2 = 0".
" GROUP BY konx2".
...
$pdf->SetFont('arial','',8);
$pdf->Cell(90,6,"Hrubá mzda, odvody, daò a celková cena práce zamestnancov za $vyb_umep / $vyb_umek","LTB",0,"L");
$pdf->Cell(0,6,"$kli_nxcf strana $strana","RTB",1,"R");


$pdf->Cell(40,5,"Priezvisko,meno Osè. ","1",0,"L");$pdf->Cell(20,5,"Odprac.hod","1",0,"R");
$pdf->Cell(20,5,"Hrubá mzda","1",0,"R");
$pdf->Cell(20,5,"FondyZamnec","1",0,"R");
$pdf->Cell(20,5,"DaòZpríjmu","1",0,"R");
$pdf->Cell(20,5,"ÈistáMzda","1",0,"R");
$pdf->Cell(20,5,"FondyZamtel","1",0,"R");
$pdf->Cell(0,5,"CenaPráce","1",1,"R");
...
if( $rtov->konx2 == 0 AND $rtov->oc > 0 )
     {
$pdf->SetFont('arial','',7);$pdf->Cell(40,5,"$rtov->prie $rtov->meno osè$rtov->oc ","0",0,"L");$pdf->SetFont('arial','',8);$pdf->Cell(20,5,"$dni","0",0,"R");
$pdf->Cell(20,5,"$rtov->hru","0",0,"R");
$pdf->Cell(20,5,"$rtov->foz","0",0,"R");
$pdf->Cell(20,5,"$rtov->dnp","0",0,"R");
$pdf->Cell(20,5,"$rtov->cst","0",0,"R");
$pdf->Cell(20,5,"$rtov->fon","0",0,"R");
...
if( $rtov->konx2 == 1998 )
     {
$pdf->Cell(40,5,"SPOLU STR $rtov->stx $rtov->nst","1",0,"L");
$pdf->Cell(20,5,"$rtov->dni","1",0,"R");
$pdf->Cell(20,5,"$rtov->hru","1",0,"R");
$pdf->Cell(20,5,"$rtov->foz","1",0,"R");
$pdf->Cell(20,5,"$rtov->dnp","1",0,"R");
$pdf->Cell(20,5,"$rtov->cst","1",0,"R");
$pdf->Cell(20,5,"$rtov->fon","1",0,"R");
...
if( $rtov->konx2 == 1999 )
     {
$pdf->Cell(40,5,"SPOLU všetko","1",0,"L");
$pdf->Cell(20,5,"$rtov->dni","1",0,"R");
$pdf->Cell(20,5,"$rtov->hru","1",0,"R");
$pdf->Cell(20,5,"$rtov->foz","1",0,"R");
$pdf->Cell(20,5,"$rtov->dnp","1",0,"R");
...
if( $i == 0 )
     {
  $text = "osc;prie,meno,titl;Odprac.hod;hru.mzda;FondyZamnec;DanZprijmu;CistaMzda;fondyZtel;CenaPrace"."\r\n"; 
  fwrite($soubor, $text);
...
$fozxx=$hlavicka->foz; $fozxx=str_replace(".",",",$fozxx); 
$dnpxx=$hlavicka->dnp; $dnpxx=str_replace(".",",",$dnpxx); 
$cstxx=$hlavicka->cst; $cstxx=str_replace(".",",",$cstxx);  

  $text = $hlavicka->oc.";".$hlavicka->prie." ".$hlavicka->meno." ".$hlavicka->titl.";";
  $text = $text.$dnixx.";".$hruxx.";".$fozxx.";".$dnpxx.";".$cstxx.";".$fonxx.";".$spoxx."\r\n";
